...
class Settings(BaseSettings):
    ...
    database_url: str = "sqlite:////tmp/brokerassist.db" if os.environ.get("VERCEL") else "sqlite:///./brokerassist.db"
    ...
    # Hugging Face Inference API for embeddings (e.g. BAAI/bge-small-en-v1.5)
    hf_api_base: str = "https://api-inference.huggingface.co"
    hf_api_key: str = Field(
        "", validation_alias=AliasChoices("BA_HF_API_KEY")
    )
    hf_embedding_model: str = Field(
        "BAAI/bge-small-en-v1.5", validation_alias=AliasChoices("BA_HF_EMBEDDING_MODEL")
    )
    
    embedding_model: str = "embeddinggemma"
    # Embedding provider selector (like marketdata_provider). "auto" = real provider when
    # configured + not use_mocks, else mock. "mock" forces the deterministic mock even when use_mocks is
    # off. "ollama" forces the real Ollama adapter. "hf" forces the Hugging Face adapter.
    embedding_provider: str = "auto"  # auto | mock | ollama | hf
    ...
    def hf_configured(self) -> bool:
        return bool(self.hf_api_base and self.hf_api_key and self.hf_embedding_model)
