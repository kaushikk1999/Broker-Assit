...
def knowledge_intent(text: str) -> str:
    """Public knowledge sub-classifier (disclosure / navigation / faq / algo / knowledge_general).

    The keyword sets are English, so callers should run this on the **translated English query** to give
    Hindi/Tamil questions the same precise labels as English. `classify()` runs it on the raw message for
    the initial fork; the RAG pipeline refines the label on `query_en` after translation."""
    return _knowledge_intent(text.lower())
