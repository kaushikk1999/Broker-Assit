...
from app.services.intent_router import knowledge_intent
...
def handle_knowledge(message: str, user_lang: str, db: DBSession, filters: dict | None = None) -> dict:
    ...
    # 1a) Refine the knowledge sub-intent on the TRANSLATED English query, so Hindi/Tamil questions get
    # the same precise disclosure/algo/navigation/faq labels as English (the keyword sets are English).
    intent = knowledge_intent(query_en)
    ...
    # 2) Query embedding (Ollama Cloud / Hugging Face in prod).
    query_vec = embedding.embed(query_retrieval, is_query=True)
    ...
    if not _is_relevant(query_en, [c.text for c in top]):
        ...
        return {"answer": NO_GROUNDING[lang], "citations": [], "intent": intent,
    ...
    for c in top:
        ...
        if doc:
            ...
            valid_context_chunks.append(c.text)

    # Fail loudly if all retrieved chunks were dangling/orphaned
    if not valid_context_chunks:
        lang = user_lang if user_lang in NO_GROUNDING else "en"
        return {"answer": NO_GROUNDING[lang], "citations": [], "intent": intent, 
                "debug": {"error": "All retrieved chunks were missing from database."}}

    # 5) Context assembly + generation (Gemma) — STRICTLY bound to verified citations.
    answer_en = llm.generate(query_en, valid_context_chunks)
