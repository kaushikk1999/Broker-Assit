    ...
    if settings.ollama_configured:
        try:
            ...
            if settings.ingestion_fail_fast:
                raise CollectionContractError(f"Ollama Cloud probe failed at startup: {e}")
            return {"phase5": "ollama_unreachable", "error": str(e), "fatal": False}
...
def run_embedding_pipeline(db: DBSession, *, fail_fast: bool | None = None,
    ...
    for doc_id in {d.id for _, d in pairs}:
        record_indexed(db, doc_id, {"backend": "mock" if settings.use_mocks else "ollama_cloud"})
